...
{
   public static class GameLogic  // it doesn't need to store data
   {
      ...
      {
         // whatever we do on the playerinfo model it will be reflected anywhere else
         // becuase we are passing the location (the same address in memory) 
         // becuase of that we choose void as return type and not a PlayerInfoModel
      ...
      public static void MarkShotResult(PlayerInfoModel player, string row, int column, bool isAHit)
      {
         foreach (var gridShot in player.ShotGrid)
         {
            if (gridShot.SpotLetter == row.ToUpper() && gridShot.SpotNumber == column)
            {
               if (isAHit)
               {
                  gridShot.Status = GridSpotStatus.Hit;
               }
               else
               {
                  gridShot.Status = GridSpotStatus.Miss;
               }
            }
         }
      }
